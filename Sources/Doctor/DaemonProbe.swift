...
public enum DaemonProbe {
    ...
    public static func reading(of installation: Installation, replyTimeout: Duration = .seconds(5)) -> DaemonReading {
        reading(from: HelperConnection(installation: installation, replyTimeout: replyTimeout))
    }

    /// What one status call over `helper` came back as.
    static func reading(from helper: HelperConnection) -> DaemonReading {
        do {
    ...
    /// refused at the first act, which a status call never makes - so a 4097 here is the
    /// signature. A service nobody holds is an invalid one (4099). Anything else is shown
    /// as what it said. [LAW:no-silent-failure]
